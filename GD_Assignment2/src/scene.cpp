...
void ScenePlay::init()
{
	...
	const auto ws = gameEngine->m_window.getSize();
	...
	m_player->cInput = std::shared_ptr<CInput>(gameEngine->cInput);
...
void ScenePlay::update(sf::Time deltaTime)
{
	this->currentFrame++;
...
{
	const auto& windowSize = gameEngine->m_window.getSize();
...
void ScenePlay::DrawImGuiUI(sf::Time deltaTime)
{
	ImGui::SFML::Update(gameEngine->m_window, deltaTime);
	...
	if (ImGui::CollapsingHeader("Debug Mouse")) {
		auto mpos = (sf::Mouse::getPosition(gameEngine->m_window));
		auto mposMapped = gameEngine->m_window.mapPixelToCoords(sf::Mouse::getPosition(gameEngine->m_window));
...
#endif // DEMO
	ImGui::SFML::Render(gameEngine->m_window);
}

void ScenePlay::sRender(sf::Time deltaTime)
{
	gameEngine->m_window.clear(sf::Color::Black);

	if (gameState.EnableSRender)
	{
		gameEngine->m_window.draw(m_background);

		// Each entity has shape component
		for (auto& e : m_entityManager.getEntities()) {
			gameEngine->m_window.draw(e->cShape->shape);
		}
		gameEngine->m_window.draw(m_player->cScore->scoreText);
	}

	DrawImGuiUI(deltaTime);
	//gameEngine->m_window.display();
}
...
void ScenePlay::sPlayerWeapon()
{
	...
	if (m_player->cInput->leftMouse && (currTime - m_player->cGun->lastSpawned).count() > m_player->cGun->coolDownTime) {
		...
		auto mpos = gameEngine->m_window.mapPixelToCoords(sf::Mouse::getPosition(gameEngine->m_window));
...
void ScenePlay::sLifetimeManagement()
{
	const auto ws = gameEngine->m_window.getSize();
...
void ScenePlay::shootEnemy(std::shared_ptr<Entity>& e)
{
	e->destroy();
